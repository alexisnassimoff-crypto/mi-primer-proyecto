...
def produce_song(song: dict, state: dict, publish: bool) -> None:
    ...
    video = atlabs_client.generate_video(
        song_id, mp3, payload["scene_prompt"], character_ref_id=payload.get("character", "suki"), aspect="16:9"
    )

    # 3) Short vertical (9:16)
    short = atlabs_client.generate_video(
        song_id, mp3, payload.get("short_scene", payload["scene_prompt"]),
        character_ref_id=payload.get("character", "suki"), aspect="9:16",
    )
    ...
    short_title = payload["youtube_title"].split(" | ")[0] + " #Shorts | Suki & Friends"
...
def make_compilation(state: dict, name: str, publish: bool) -> None:
    ...
    youtube_client.upload_video(
        out, f"{name} 🌟 Kids Songs | Suki & Friends",
        "A cozy collection of learning and lullaby songs with Suki & Friends. 🌟 Subscribe!",
        ["kids songs compilation", "nursery rhymes", "learning songs", "suki and friends"],
        privacy=privacy,
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Suki & Friends — produce la proxima pieza.")
    ...
    print(f"== Suki & Friends pipeline — modo {mode} ==")
